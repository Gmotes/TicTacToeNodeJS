...
socket = io.connect("http://localhost:3000"); // 3000 portuna connect olduk
 nick = $("#txtNick").val(); // #nickinput'un valuesini al
socket.emit('login',{ //socket.emit ('mesajgonder') aracýlýðýyla server.js'de socket.on da bizi bekleyen komutu JSON olarak yolladýk.
                    'uid' : nick // Hem mesajý hem uid yani kullanýcý kimliðini socket.on'a aktardýk.
});
